...
/// <summary>
/// Implements race overview endpoints from the frgle API.
/// </summary>
public class RaceApiService : IRaceApiService
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="RaceApiService"/> class.
    /// </summary>
    /// <param name="configuration">Central app configuration.</param>
    public RaceApiService(IAppConfiguration? configuration = null)
    {
    ...
    /// <inheritdoc />
    public async Task<List<RaceDto>?> FindAllAsync(int eventId, CancellationToken ct = default)
    {
        return await _httpClient.GetFromJsonAsync<List<RaceDto>>($"/api/{eventId}/races", ct);
    }
}
